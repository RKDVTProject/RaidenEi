...
export async function participantsUpdate({ id, participants, action }) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/9364c1aaad0e5c71618c3.jpg'
                    try {
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  let welcom = 'https://telegra.ph/file/37351ff90ccbdc669c713.jpg'

  let godbye = 'https://telegra.ph/file/37351ff90ccbdc669c713.jpg'
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? pp : pp)).buffer(), 'Group Messege', text, action == 'add' ? 'Selamat Datang 😇' : 'Sampai Jumpa 👋', action === 'add' ? '.intro' : 'heheh 🗿', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'tautanwa',
    mediaType: tautanwa, 
